...
=== ATLAS_BUNDLE_JSON ===
{
  ...
  "core_principles": [
    {
      "id": "principle-short-id",
      "title": "判断基準・大切にしている観点",
      "summary": "本人が明言・採用している判断基準や観点（推測でなく発言・選択から見出す）。",
      "importance": 0.0,
      "confidence": 0.0,
      "friction_level": 0.0,
      "dilemma_level": 0.0,
      "exploration_level": 0.0,
      "keywords": ["keyword"],
      "evidence": ["採用が分かる具体的な言及・選択（情報源つき）"],
      "counter_evidence": ["この基準が当てはまらない例外事象"],
      "structural_note": "この基準がどう働いているかの事実整理（推測は避ける）"
    }
  ],
  "observed_patterns": [
    {
      "id": "pattern-short-id",
      "title": "観察されたパターン",
      "summary": "複数の話題で繰り返し見られる相談形式・行動・判断材料。内面や性格は推測しない。",
      "importance": 0.0,
      "confidence": 0.0,
      "friction_level": 0.0,
      "dilemma_level": 0.0,
      ...
      "keywords": ["keyword"],
      "evidence": ["情報源と具体的な根拠"],
      "counter_evidence": ["例外、まだ分からない点"],
      "review_prompt": "本人が見直すための中立的な問い"
    }
  ],
  "open_questions": [
    {
      ...
      "title": "未整理の問い",
      "summary": "参照情報に明示されている迷い・検討中の選択肢・未決着の論点を整理する。",
      "related_ids": ["topic-or-principle-id"],
      "importance": 0.0,
      "confidence": 0.0,
      "friction_level": 0.0,
      "dilemma_level": 0.0,
  ...
  "tensions": [
    {
      "id": "tension-short-id",
      "title": "A vs B",
      "summary": "本人が明示した『AしたいがBの制約』『XとYで迷う』構造のみ。",
      "from": "existing-id",
      "to": "existing-id",
      "strength": 0.0,
      "status": "active/unresolved/accepted",
      "evidence": ["迷い・矛盾として語られた事実（情報源つき）"],
      "counter_evidence": ["例外、まだ分からない点"],
      "question": "この構造を本人が見直すための事実確認の問い（診断ではない）"
    }
  ],
  "links": [
    {
      "source": "existing-id",
      "target": "existing-id",
      "type": "related_to/supports/complicates",
      "strength": 0.0,
      "reason": "参照情報に基づく関係の理由"
    }
  ]
}
...
=== INPUT_MATERIAL_END ===`;
  ...
  async function copyPrompt() {
    ...
    if (!ok) {
      refs.promptPreview.focus();
      refs.promptPreview.select();
    }
    flash(refs.copyPromptButton, ok ? "コピー済み" : "手動コピー");
    setStatus(ok ? "good" : "warn", ok ? "コピー済み" : "全選択しました");
  }
  ...
  function setStatus(kind, text) {
    refs.parseStatus.className = `status-badge ${kind}`;
    refs.parseStatus.textContent = text;
  }

  function importText(text, source) {
    try {
      const parsed = extractJson(text);
      loadAtlas(parsed, source);
      setStatus("good", "読み込み完了");
    } catch (error) {
      setStatus("warn", "JSONを確認してください");
      refs.healthBadge.className = "status-badge warn";
      refs.healthBadge.textContent = "Parse error";
      refs.healthSummary.textContent = error.message;
    }
  }
  ...
  async function loadSample() {
    try {
      ...
      loadAtlas(await response.json(), "sample-atlas.json");
      setStatus("good", "サンプル読込済み");
  ...
  function extractJson(text) {
    ...
    if (marked) return JSON.parse(marked[1]);
  ...
  function normalizeAtlas(input, sourceName) {
    ...
    const beliefs = v2 ? [
      ...arr(input.core_principles).map((x) => norm({ ...x, kind: "belief", source_type: "core_principle", interpretation: x.structural_note || x.interpretation })),
      ...arr(input.observed_patterns).map((x) => norm({ ...x, kind: "belief", source_type: "observed_pattern", interpretation: x.review_prompt || x.interpretation }))
    ] : arr(input.latent_beliefs || input.beliefs).map((x) => norm({ ...x, kind: "belief" }));
    ...
    return {
      version: input.version || "atlas_bundle_v1",
      meta: { title: input.meta?.title || input.title || "Imported Atlas", note: input.meta?.note || "This is a hypothesis map, not a diagnosis.", source_name: sourceName, imported_at: new Date().toISOString() },
      thought_nodes: nodes, latent_beliefs: beliefs, tensions, edges: v2 ? arr(input.links || input.edges) : arr(input.edges), user_edits: input.user_edits || {}
    };
  }

  function norm(item) {
    return { ...item, shadow_level: item.shadow_level ?? item.friction_level ?? 0, tension_level: item.tension_level ?? item.dilemma_level ?? 0, exploration_level: item.exploration_level ?? 0, importance: item.importance ?? 0, confidence: item.confidence ?? 0 };
  }
  function arr(value) { return Array.isArray(value) ? value : []; }
  function num(value) { const n = Number(value); return Number.isFinite(n) ? n : 0; }

  function loadAtlas(input, sourceName) {
    state.atlas = normalizeAtlas(input, sourceName);
    state.cards = collectCards(state.atlas);
    state.selected = state.cards[0] || null;
    localStorage.setItem(STORAGE_KEY, JSON.stringify(state.atlas));
    renderHealth(checkHealth(state.atlas));
    renderAll();
    showView("tours");
  }

  function collectCards(atlas) {
    return [
      ...arr(atlas.thought_nodes).map((x) => ({ ...x, kind: "node" })),
      ...arr(atlas.latent_beliefs).map((x) => ({ ...x, kind: "belief" })),
      ...arr(atlas.tensions).map((x) => ({ ...x, kind: "tension" }))
    ];
  }

  function checkHealth(atlas) {
    const messages = [];
    const ids = new Set();
    collectCards(atlas).forEach((item) => {
      if (!item.id) messages.push("idがないカードがあります。");
      if (item.id && ids.has(item.id)) messages.push(`重複id: ${item.id}`);
      if (item.id) ids.add(item.id);
      if (!item.title) messages.push(`${item.id || "unknown"} にtitleがありません。`);
      if (!item.summary) messages.push(`${item.id || item.title || "unknown"} にsummaryがありません。`);
      if (!arr(item.evidence).length) messages.push(`${item.id || item.title || "unknown"} にevidenceがありません。`);
      if (!arr(item.counter_evidence).length && item.kind !== "tension") messages.push(`${item.id || item.title || "unknown"} にcounter_evidenceがありません。`);
    });
    arr(atlas.edges).forEach((edge) => {
      if (!ids.has(edge.source)) messages.push(`edge.source ${edge.source} が存在しません。`);
      if (!ids.has(edge.target)) messages.push(`edge.target ${edge.target} が存在しません。`);
    });
    arr(atlas.tensions).forEach((tension) => {
      if (tension.from && !ids.has(tension.from)) messages.push(`tension.from ${tension.from} が存在しません。`);
      if (tension.to && !ids.has(tension.to)) messages.push(`tension.to ${tension.to} が存在しません。`);
    });
    return messages;
  }

  function renderHealth(messages) {
    refs.healthList.innerHTML = "";
    refs.copyRepairPromptButton.hidden = !messages.length;
    refs.healthBadge.className = `status-badge ${messages.length ? "warn" : "good"}`;
    refs.healthBadge.textContent = messages.length ? `${messages.length}件` : "OK";
    refs.healthSummary.textContent = messages.length ? "修正候補があります。" : "基本チェックを通過しました。";
    messages.forEach((message) => { const li = document.createElement("li"); li.textContent = message; refs.healthList.append(li); });
  }

  function renderTabs() {
    refs.cardTabs.innerHTML = "";
    cardTabs.forEach((tab) => refs.cardTabs.append(tabButton(tab, state.cardFilter, (id) => { state.cardFilter = id; renderTabs(); renderCards(); })));
    refs.tourTabs.innerHTML = "";
    tourTabs.forEach((tab) => refs.tourTabs.append(tabButton(tab, state.tourFilter, (id) => { state.tourFilter = id; renderTabs(); renderTours(); })));
  }
  ...
  function renderTours() {
    ...
    const items = [...state.cards].sort((a, b) => score(b, state.tourFilter) - score(a, state.tourFilter)).slice(0, 8);
    if (!items.length) return empty(refs.tourList, "Atlasを読み込むとTourが表示されます。");
    items.forEach((item, index) => refs.tourList.append(cardEl(item, `${index + 1} / score ${score(item, state.tourFilter).toFixed(2)}`)));
  }

  function renderCards() {
    refs.cardList.innerHTML = "";
    let items = state.cards.filter((item) => state.cardFilter === "all" || item.kind === state.cardFilter);
    if (state.query) items = items.filter((item) => [item.title, item.summary, ...(item.keywords || [])].join(" ").toLowerCase().includes(state.query));
    refs.cardCount.textContent = `${items.length}件`;
    if (!items.length) return empty(refs.cardList, "カードがありません。");
    items.forEach((item) => refs.cardList.append(cardEl(item, kindLabel(item.kind))));
  }

  function cardEl(item, meta) {
    const el = refs.cardTemplate.content.firstElementChild.cloneNode(true);
    el.querySelector(".card-kind").textContent = item.source_type ? `${kindLabel(item.kind)} / ${item.source_type}` : kindLabel(item.kind);
    el.querySelector(".card-title").textContent = item.title || item.id || "Untitled";
    el.querySelector(".card-summary").textContent = item.summary || "";
    el.querySelector(".card-flags").textContent = (item.keywords || []).slice(0, 4).join(" / ");
    el.querySelector(".card-meta").textContent = meta;
    el.addEventListener("click", () => { state.selected = item; renderDetail(); showView("detail"); });
    return el;
  }

  function renderDetail() {
    const item = state.selected;
    if (!item) { refs.detailPanel.className = "detail-panel empty-state"; refs.detailPanel.innerHTML = "<p>まだカードが選択されていません。</p>"; return; }
    refs.detailPanel.className = "detail-panel";
    refs.detailHint.textContent = item.id || "";
    refs.detailPanel.innerHTML = `
      <span class="card-kind">${esc(kindLabel(item.kind))}</span>
      <h3>${esc(item.title || "Untitled")}</h3>
      <p>${esc(item.summary || "")}</p>
      <dl class="score-grid">
        <div><dt>importance</dt><dd>${fmt(item.importance)}</dd></div>
        <div><dt>confidence</dt><dd>${fmt(item.confidence)}</dd></div>
        <div><dt>friction</dt><dd>${fmt(item.shadow_level)}</dd></div>
        <div><dt>dilemma</dt><dd>${fmt(item.tension_level)}</dd></div>
        <div><dt>explore</dt><dd>${fmt(item.exploration_level)}</dd></div>
      </dl>
      ${list("Evidence", item.evidence)}${list("Counter evidence", item.counter_evidence)}
      ${item.question ? `<h4>Question</h4><p>${esc(item.question)}</p>` : ""}
      ${item.interpretation ? `<h4>Review note</h4><p>${esc(item.interpretation)}</p>` : ""}`;
  }
  ...
  function score(item, mode) {
    if (mode === "shadow") return num(item.shadow_level) * 0.6 + num(item.tension_level) * 0.2 + num(item.importance) * 0.2;
    if (mode === "recurring") return num(item.importance) * 0.45 + num(item.confidence) * 0.45 + num(item.tension_level) * 0.1;
    if (mode === "emerging") return num(item.exploration_level) * 0.55 + (1 - num(item.confidence)) * 0.2 + num(item.importance) * 0.25;
    return num(item.importance) * 0.45 + num(item.exploration_level) * 0.3 + num(item.tension_level) * 0.25;
  }
